...
class DappradarSpider(scrapy.Spider):
    name = "dappradar"
    ...
    custom_settings = {
        ...
        'LOG_FILE': 'dappradar.log',
        'COOKIES_ENABLED': False,  # We handle cookies manually
    }
    ...
    def start_requests(self):
        ...
        for page in range(1, 11):
            # Double encode params (base64 twice)
            params_dict = {
            ...
            encoded_twice = 'UkdGd2NGSmhaR0Z5WTNWeWNtVnVZM2s5VlZORUpuTnZjblE5ZFdGM1EyOTFiblFtYjNKa1pYSTlaR1Z6WXlaeVlXNW5aVDB5TkdnbWNtVnpkV3gwYzFCbGNsQmhaMlU5TlRBbWNHRm5aVDB5Sm1OaGRHVm5iM0o1U1dSYlhUMHhKbVY0WTJ4MVpHVmtSR0Z3Y0Vsa1BUUXdNREV6'
            ...
            url_with_params = f"{url}?params={encoded_twice}"
    ...
    def parse(self, response):
        ...
        try:
            ...
            self.logger.info(f"Page {page}: Found {len(dapps)} dapps")
            ...
            for dapp in dapps:
                ...
                yield {
                    ...
                    'mobileFriendly': dapp.get('mobileFriendly'),
                    'featured': dapp.get('featured'),
                    'premium': dapp.get('premium'),
                    'listedAt': dapp.get('listedAt'),
                    # Statistics
                    'totalBalanceInFiat': stats.get('totalBalanceInFiat'),
                    'totalBalanceChange': stats.get('totalBalanceChange'),
                    'totalVolumeInFiat': stats.get('totalVolumeInFiat'),
                    'totalVolumeChange': stats.get('totalVolumeChange'),
                    'transactionCount': stats.get('transactionCount'),
                    'transactionCountChange': stats.get('transactionCountChange'),
                    'uawCount': stats.get('uawCount'),
                    'uawCountChange': stats.get('uawCountChange'),
                    'newUAWCount': stats.get('newUAWCount'),
                }
